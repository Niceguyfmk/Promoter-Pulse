...
export function AssignedSurveyForm({
  ...
  const { trackOperation } = useLoading();
  ...
  const surveyModel = useMemo(() => {
    ...
    // Keep SurveyJS completion aligned with the outer visit report save action.
    nextModel.onCompleting.add((_, options) => {
      options.allowComplete = false;

      const submitBtn = document.querySelector(
        `button[form="${formId}"][value="save"]`
    ...
    nextModel.onUploadFiles.add((_, options) => {
      const promises = options.files.map(async (file) => {
        ...
        const result = await trackOperation(uploadSurveyFileAction(formData), {
          kind: "upload",
    ...
    return nextModel;
  }, [formId, initialAnswers, reportId, selectedForm, storeId, trackOperation]);
  ...
  }, [effectiveSelectedFormId, onChange, selectedForm?.name, surveyModel]);
